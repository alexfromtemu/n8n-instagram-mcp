...
You can also run the MCP server in a Docker container for better isolation and portability.
...
- **Session persistence**: Session files are stored in `./sessions/` directory
- **Media downloads**: Downloads are saved to `./downloads/` directory
- **Isolated environment**: All dependencies are contained in the container
